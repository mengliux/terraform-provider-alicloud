	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				ImportStateVerifyIgnore: []string{"account_privilege", "collate", "ctype"},
			},
			{
				Config: testAccConfig(map[string]interface{}{
...
func TestAccAlicloudPolarDBOracleDatabase_update(t *testing.T) {
	var database *polardb.Database
	resourceId := "alicloud_polardb_database.default"
	...
	testAccCheck := rac.resourceAttrMapUpdateSet()
	name := "tf-testAccDBdatabase_basic"
	testAccConfig := resourceTestAccConfigFunc(resourceId, name, resourcePolarDBOracleDatabaseConfigDependence)
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheck(t)
		},
...
func resourcePolarDBOracleDatabaseConfigDependence(name string) string {
	return fmt.Sprintf(`
	%s
	variable "name" {
		default = "%s"
	...
	resource "alicloud_polardb_cluster" "instance" {
		...
		db_version = "11"
		pay_type = "PostPaid"
		db_node_class = "polar.o.x4.large"
		vswitch_id = local.vswitch_id
		description = "${var.name}"
	}
