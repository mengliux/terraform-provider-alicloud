...
func (s *PolarDBService) DescribePolarDBClusterMigration(id string) (object map[string]interface{}, err error) {
	var response map[string]interface{}
	conn, err := s.client.NewPolarDBClient()
	if err != nil {
		return nil, WrapError(err)
...
func (s *PolarDBService) PolarDBClusterMigrationStateRefreshFunc(id string, failStates []string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {

		object, err := s.DescribePolarDBClusterMigration(id)
		if err != nil {
			if NotFoundError(err) {
				// Set this to nil as if we didn't find anything.
				return nil, "", nil
		...
		for _, failState := range failStates {
			if fmt.Sprint(object["MigrationStatus"]) == failState {
				return object, fmt.Sprint(object["MigrationStatus"]), WrapError(Error(FailedToReachTargetStatus, fmt.Sprint(object["MigrationStatus"])))
			}
		}
		return object, fmt.Sprint(object["MigrationStatus"]), nil
	}
}
